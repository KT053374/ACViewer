...
namespace ACE.Server.WorldObjects
{
    ...
    partial class WorldObject
    {
        ...
        public void InitializeGenerator()
        {
            ...
            {
                Console.WriteLine($"0x{Guid.Full} {Name}.InitializeGenerator: {WeenieClassName} ({WeenieClassId}) MaxGeneratedObjects = {MaxGeneratedObjects} | InitGeneratedObjects = {InitGeneratedObjects}. Setting MaxGeneratedObjects = InitGeneratedObjects");
            ...
            while (true)
            {
                ...
                for (var i = 0; i < GeneratorProfiles.Count; i++)
                {
                    ...
                    if (profile.RegenLocationType.HasFlag(RegenLocationType.Treasure))
                    {
                        ...
                        {
                            Console.WriteLine($"0x{Guid} {Name}.SelectProfilesInit(): profile[{i}].RegenLocationType({profile.RegenLocationType}), profile.Biota.WCID({profile.Biota.WeenieClassId}), profile.Biota.InitCreate({profile.Biota.InitCreate}) > 1, set to 1. WCID: {WeenieClassId} - LOC: {Location.ToLOCString()}");
                            profile.Biota.InitCreate = 1;
                        }

                        if (profile.Biota.MaxCreate > 1)
                        {
                            Console.WriteLine($"0x{Guid} {Name}.SelectProfilesInit(): profile[{i}].RegenLocationType({profile.RegenLocationType}), profile.Biota.WCID({profile.Biota.WeenieClassId}), profile.Biota.MaxCreate({profile.Biota.MaxCreate}) > 1, set to 1. WCID: {WeenieClassId} - LOC: {Location.ToLOCString()}");
                ...
                if (loopcount > 1000)
                {
                    Console.WriteLine($"0x{Guid} {Name}.SelectProfilesInit(): loopcount > 1000, aborted. WCID: {WeenieClassId} - LOC: {Location.ToLOCString()}");
                    CurrentlyPoweringUp = false;
                    return;
                }
            }
