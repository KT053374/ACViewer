...
namespace ACE.Server.Managers
{
    public static class EventManager
    {
        ...
        public static void Initialize()
        {
            ...
            Console.WriteLine("EventManager Initalized.");
        }
        ...
        public static bool StartEvent(string e, WorldObject source, WorldObject target)
        {
            ...
            Console.WriteLine($"[EVENT] {(source == null ? "SYSTEM" : $"{source.Name} (0x{source.Guid}|{source.WeenieClassId})")}{(target == null ? "" : $", triggered by {target.Name} (0x{target.Guid}|{target.WeenieClassId}),")} started an event: {evnt.Name}{((int)state == evnt.State ? (source == null ? ", which is the default state for this event." : ", which had already been started.") : "")}");
        ...
        public static bool StopEvent(string e, WorldObject source, WorldObject target)
        {
            ...
            Console.WriteLine($"[EVENT] {(source == null ? "SYSTEM" : $"{source.Name} (0x{source.Guid}|{source.WeenieClassId})")}{(target == null ? "" : $", triggered by {target.Name} (0x{target.Guid}|{target.WeenieClassId}),")} stopped an event: {evnt.Name}{((int)state == evnt.State ? (source == null ? ", which is the default state for this event." : ", which had already been stopped.") : "")}");
