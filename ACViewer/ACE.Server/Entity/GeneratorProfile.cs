...
namespace ACE.Server.Entity
{
    ...
    public class GeneratorProfile
    {
        ...
        /// </summary>
        public PropertiesGenerator Biota { get; set; }
        ...
        public void ProcessQueue()
        {
            ...
            while (index < SpawnQueue.Count)
            {
                ...
                else
                {
                    ...
                    Console.WriteLine($"GeneratorProfile: objects enqueued for {Generator.Name}, but MaxCreate({MaxCreate}) already reached!");
                }
        ...
        public List<WorldObject> Spawn()
        {
            ...
            else
            {
                ...
                if (wo == null)
                {
                    Console.WriteLine($"{Generator.Name}.Spawn(): failed to create wcid {Biota.WeenieClassId}");
                    return null;
                }
        ...
        public bool Spawn_Container(WorldObject obj)
        {
            ...
            if (!success)
                Console.WriteLine($"{Generator.Name}.Spawn_Container({obj.Name}) - failed to add to container inventory");
        ...
        public bool Spawn_Shop(WorldObject obj)
        {
            ...
            {
                Console.WriteLine($"{Generator.Name}.Spawn_Shop({obj.Name}) - generator is not a vendor type");
                return false;
            }
        ...
        public List<WorldObject> TreasureGenerator()
        {
            ...
            else
            {
                ...
                {
                    Console.WriteLine($"{Generator.Name}.TreasureGenerator(): couldn't find death treasure or wielded treasure for ID {Biota.WeenieClassId}");
                    return null;
                }
            }
        }
        ...
        public void RemoveTreasure()
        {
            ...
            if (container == null)
            {
                Console.WriteLine($"{Generator.Name}.RemoveTreasure(): container not found");
                return;
            }
            foreach (var spawned in Spawned.Keys)
            {
                ...
                {
                    Console.WriteLine($"{Generator.Name}.RemoveTreasure(): couldn't find {inventoryObjGuid}");
                    continue;
                }
        ...
        public void NotifyGenerator(ObjectGuid target, RegenerationType eventType)
        {
            ...
            if (whenCreate != adjWhenCreate)
                Console.WriteLine($"0x{Generator.Guid}:{Generator.Name}({Generator.WeenieClassId}).GeneratorProfile[{LinkId}].NotifyGenerator: RegenerationType = {eventType.ToString()}, WhenCreate = {whenCreate.ToString()}, Using {adjWhenCreate.ToString()} as WhenCreate instead");
